...
function report(job, appsettings) {
  ...
  fs.writeFile(
    appsettings.GeneralSettings.ReportOutputFolder + '/' + job.Parameters.JobName + '_' + dt + '.html',
    htmlString,
    function (err) {
      if (err) {
...
function embedAuthorDataIntoHtml(html) {
  ...
  if (_report.Authors !== undefined) {
    for (var i = 0; i < _report.Authors.length; i++) {
      results.CommitStat.push({ Author: _report.Authors[i].Name, TotalCommits: _report.Authors[i].TotalCommits });
      results.LocStat.push({
        Author: _report.Authors[i].Name,
        TotalLinesAdded: _report.Authors[i].loc.AddedLines,
        TotalLinesDeleted: _report.Authors[i].loc.DeletedLines,
        TotalTotalLines: _report.Authors[i].loc.TotalLines
      });
    }
  }
  $('#AuthorData').text(_report.GeneralInformations);
